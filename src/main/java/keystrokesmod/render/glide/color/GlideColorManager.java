package keystrokesmod.render.glide.color;
...
public class GlideColorManager {
    public static final String[] COLOR_NAMES = new String[] {
            "Default", "Evening Sunshine", "Light Orange", "Reef", "Amin", "Magic",
            "Mango Pulp", "Moon Purple", "Aqualicious", "Stripe", "Shifter", "Quepal",
            "Orca", "Sublime Vivid", "Moon Asteroid", "Summer Dog", "Pink Flavour",
            "Sin City Red", "Timber", "Pinot Noir", "Dirty Fog", "Piglet", "Little Leaf",
            "Nelson", "Turquoise flow", "Purplin", "Martini", "SoundCloud", "Inbox",
            "Amethyst", "Blush", "Mocha Rose", "Muted Ocean", "Algae", "Greys",
            "Pandas", "Flame"
    };
    ...
    private GlideTheme theme;

    public GlideColorManager() {

        theme = GlideTheme.LIGHT;
    ...
    public GlideTheme getTheme() {
        return theme;
    }

    public void setTheme(GlideTheme theme) {
        this.theme = theme;
    }
    ...
    public void syncFromSettings() {
        if (Settings.glideTheme != null) {
            int themeIndex = (int) Settings.glideTheme.getInput();
            theme = GlideTheme.getThemeById(themeIndex);
        }

        if (Settings.glideAccent != null) {
            int accentIndex = (int) Settings.glideAccent.getInput();
            String[] options = Settings.glideAccent.getOptions();
            if (accentIndex >= 0 && accentIndex < options.length) {
                currentColor = getColorByName(options[accentIndex]);
            }
        }
    }
}
