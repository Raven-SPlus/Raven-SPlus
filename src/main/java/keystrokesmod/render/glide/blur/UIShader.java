...
import keystrokesmod.render.glide.blur.uniform.*;
...
public final class UIShader {
    ...
    public void init() throws IOException {
        ...
        if (GL20.glGetProgrami(pid, GL20.GL_LINK_STATUS) == GL11.GL_FALSE) {
            System.err.println("[Glide] Failed to link program: " + GL20.glGetProgramInfoLog(pid, GL20.GL_INFO_LOG_LENGTH));
        }

        if (GL20.glGetProgrami(pid, GL20.GL_VALIDATE_STATUS) == GL11.GL_FALSE) {
            System.err.println("[Glide] Failed to validate program: " + GL20.glGetProgramInfoLog(pid, GL20.GL_INFO_LOG_LENGTH));
        }
    ...
    private int createShader(String src, int type) throws IOException {
        ...
        if (GL20.glGetShaderi(id, GL20.GL_COMPILE_STATUS) == GL11.GL_FALSE) {
            System.err.println("[Glide] Failed to compile shader: " + GL20.glGetShaderInfoLog(id, GL20.GL_INFO_LOG_LENGTH));
        }
