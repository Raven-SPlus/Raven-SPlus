package keystrokesmod.render.glide.nanovg.asset;
...
public class NvgAssetManager {
    ...
    public boolean loadImage(long nvg, ResourceLocation location) {
        if (!imageCache.containsKey(location.getResourcePath())) {
            ...
            ByteBuffer image = NvgIOUtils.resourceToByteBuffer(location);
            if (image == null) {
                return false;
            }
            ...
            imageCache.put(location.getResourcePath(), new NVGAsset(
                    NanoVG.nvgCreateImageRGBA(nvg, width[0], height[0],
                            NanoVG.NVG_IMAGE_REPEATX | NanoVG.NVG_IMAGE_REPEATY | NanoVG.NVG_IMAGE_GENERATE_MIPMAPS, buffer),
                    width[0], height[0]));
        }
        return true;
    }
    ...
    public boolean loadImage(long nvg, File file) {
        if (!imageCache.containsKey(file.getName())) {
            ...
            ByteBuffer image = NvgIOUtils.resourceToByteBuffer(file);
            if (image == null) {
                return false;
            }
            ...
            imageCache.put(file.getName(), new NVGAsset(
                    NanoVG.nvgCreateImageRGBA(nvg, width[0], height[0],
                            NanoVG.NVG_IMAGE_REPEATX | NanoVG.NVG_IMAGE_REPEATY | NanoVG.NVG_IMAGE_GENERATE_MIPMAPS, buffer),
                    width[0], height[0]));
        }
    ...
    public boolean loadSvg(long nvg, ResourceLocation location, float width, float height) {
        ...
        if (!svgCache.containsKey(name)) {
            try {
                ...
                svgCache.put(name, new NVGAsset(
                        NanoVG.nvgCreateImageRGBA(nvg, w, h,
                                NanoVG.NVG_IMAGE_REPEATX | NanoVG.NVG_IMAGE_REPEATY | NanoVG.NVG_IMAGE_GENERATE_MIPMAPS, image),
                        w, h));

                return true;
            } catch (Exception e) {
                System.err.println("[Glide] Failed to load svg: " + e.getMessage());
                e.printStackTrace();
                return false;
            }
        }
