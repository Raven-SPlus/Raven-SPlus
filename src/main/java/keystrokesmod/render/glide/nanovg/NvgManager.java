...
import keystrokesmod.render.glide.nanovg.asset.NvgAssetManager;
import keystrokesmod.render.glide.nanovg.font.NvgFont;
import keystrokesmod.render.glide.nanovg.font.NvgFontManager;
...
public class NvgManager {
    ...
    private NvgFontManager fontManager;
    private NvgAssetManager assetManager;

    public NvgManager() {
        if (!NvgNativeBootstrap.ensureLoaded()) {
            Throwable failure = NvgNativeBootstrap.getFailure();
            throw new IllegalStateException("Failed to load Glide NanoVG natives", failure);
        }
        ...
        fontManager = new NvgFontManager();
        fontManager.init(nvg);

        assetManager = new NvgAssetManager();
    }
    ...
    public void drawRoundedRectVarying(float x, float y, float width, float height,
                                        float topLeftRadius, float topRightRadius,
                                        float bottomLeftRadius, float bottomRightRadius, Color color) {
        NanoVG.nvgBeginPath(nvg);
    ...
    public void drawGradientRect(float x, float y, float width, float height, Color color1, Color color2) {
        ...
        NanoVG.nvgFillColor(nvg, nvgColor1);
        NanoVG.nvgFillColor(nvg, nvgColor2);
        NanoVG.nvgFillPaint(nvg, NanoVG.nvgLinearGradient(nvg,
                x + width / 2 - (max / 2) * NvgColorUtils.cos(tick),
                y + height / 2 - (max / 2) * NvgColorUtils.sin(tick),
                x + width / 2 + (max / 2) * NvgColorUtils.cos(tick),
                y + height / 2 + (max + 2f) * NvgColorUtils.sin(tick),
                nvgColor1, nvgColor2, bg));
        NanoVG.nvgFill(nvg);
    }
    ...
    public void drawGradientOutlineRoundedRect(float x, float y, float width, float height, float radius, float strokeWidth, Color color1, Color color2) {
        ...
        NanoVG.nvgFillColor(nvg, nvgColor2);
        NanoVG.nvgStrokeWidth(nvg, strokeWidth);
        NanoVG.nvgStrokePaint(nvg, NanoVG.nvgLinearGradient(nvg,
                x + width / 2 - (max / 2) * NvgColorUtils.cos(tick),
                y + height / 2 - (max / 2) * NvgColorUtils.sin(tick),
                x + width / 2 + (max / 2) * NvgColorUtils.cos(tick),
                y + height / 2 + (max + 2f) * NvgColorUtils.sin(tick),
    ...
    public void drawGradientShadow(float x, float y, float width, float height, float radius, Color color1, Color color2) {
        ...
        for (float f = 10; f > 0; f--) {
            drawGradientOutlineRoundedRect(x - (f / 2), y - (f / 2), width + f, height + f, radius + 2, f,
                    NvgColorUtils.applyAlpha(color1, alpha), NvgColorUtils.applyAlpha(color2, alpha));
            alpha += 3;
        }
    }
    ...
    public void drawRoundedGlow(float x, float y, float width, float height, float radius, Color color1, int strength) {
        ...
        for (float f = strength; f > 0; f--) {
            drawGradientOutlineRoundedRect(x - (f / 2), y - (f / 2), width + f, height + f, radius + 2, f,
                    NvgColorUtils.applyAlpha(color1, alpha), NvgColorUtils.applyAlpha(color1, alpha));
            alpha += 2;
        }
    }
    ...
    public void drawText(String text, float x, float y, Color color, float size, NvgFont font) {
        y += size / 2;
        NanoVG.nvgBeginPath(nvg);
    ...
    public void drawTextGlowing(String text, float x, float y, Color color, float blurRadius, float size, NvgFont font) {
        drawTextGlowingBg(text, x, y, color, size, blurRadius, font);
        drawText(text, x, y, color, size, font);
    }

    private void drawTextGlowingBg(String text, float x, float y, Color color, float size, float blurRadius, NvgFont font) {
        y += size / 2;
        NanoVG.nvgBeginPath(nvg);
        NanoVG.nvgFontSize(nvg, size);
        NanoVG.nvgFontFace(nvg, font.getName());
    ...
    public void drawTextBox(String text, float x, float y, float maxWidth, Color color, float size, NvgFont font) {
        y += size / 2;
        NanoVG.nvgBeginPath(nvg);
        NanoVG.nvgFontSize(nvg, size);
        NanoVG.nvgFontFace(nvg, font.getName());
    ...
    public void drawCenteredText(String text, float x, float y, Color color, float size, NvgFont font) {
        int textWidth = (int) getTextWidth(text, size, font);
        drawText(text, x - (textWidth >> 1), y, color, size, font);
    }

    public float getTextWidth(String text, float size, NvgFont font) {
    ...
    public float getTextHeight(String text, float size, NvgFont font) {
        float[] bounds = new float[4];
        NanoVG.nvgFontSize(nvg, size);
        NanoVG.nvgFontFace(nvg, font.getName());
        NanoVG.nvgTextBounds(nvg, 0, 0, text, bounds);
    ...
    public float getTextBoxHeight(String text, float size, NvgFont font, float maxWidth) {
        float[] bounds = new float[4];
        NanoVG.nvgFontSize(nvg, size);
        NanoVG.nvgFontFace(nvg, font.getName());
    ...
    public void drawPlayerHead(ResourceLocation location, float x, float y, float width, float height, float radius, float alpha) {
        ...
        if (assetManager.loadImage(nvg, texture, width, height)) {
            ...
            NanoVG.nvgImagePattern(nvg, x - width / 4 * sizeMultiplier / 2, y - height / 4 * sizeMultiplier / 2,
                    width * sizeMultiplier, height * sizeMultiplier, 0, image, alpha, p);
            NanoVG.nvgBeginPath(nvg);
            NanoVG.nvgRoundedRect(nvg, x, y, width, height, radius);
            NanoVG.nvgFillPaint(nvg, p);
            NanoVG.nvgFill(nvg);
            ...
            NanoVG.nvgImagePattern(nvg, x - width * 3.25F * sizeMultiplier / 2, y - height / 4 * sizeMultiplier / 2,
                    width * sizeMultiplier, height * sizeMultiplier, 0, image, alpha, p);
            NanoVG.nvgBeginPath(nvg);
            NanoVG.nvgRoundedRect(nvg, x, y, width, height, radius);
            NanoVG.nvgFillPaint(nvg, p);
    ...
    public NvgAssetManager getAssetManager() {
        return assetManager;
    }
