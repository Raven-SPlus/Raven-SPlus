...
public class RenderUtils {
    ...
    private static final int SHADOW_CACHE_LIMIT = 256;
    private static final Map<Integer, Integer> shadowCache = new LinkedHashMap<Integer, Integer>(16, 0.75f, true) {
        @Override
        protected boolean removeEldestEntry(Map.Entry<Integer, Integer> eldest) {
            if (size() > SHADOW_CACHE_LIMIT) {
    ...
    public static void drawOutline(float x, float y, float x2, float y2, float lineWidth, int color) {
        // Optional GPU-friendly path: use Tessellator instead of immediate mode
        try {
            if (Settings.gpuUiRoundedRects != null && Settings.gpuUiRoundedRects.isToggled()) {
    ...
    public static void d3p(double x, double y, double z, double radius, int sides, float lineWidth, int color, boolean chroma) {
        ...
        try {
            if (Settings.gpu3DRendering != null && Settings.gpu3DRendering.isToggled()) {
                ...
                Tessellator tessellator = Tessellator.getInstance();
                WorldRenderer worldrenderer = tessellator.getWorldRenderer();
                worldrenderer.begin(GL11.GL_LINES, chroma ? DefaultVertexFormats.POSITION_COLOR : DefaultVertexFormats.POSITION);
                ...
                for (int i = 0; i < sides * 2; ++i) {
                    if (chroma) {
                        ...
                        double angle = 6.283185307179586D * (double) i / (double) sides + Math.toRadians(180.0D);
                        worldrenderer.pos(x + Math.cos(angle) * radius, y, z + Math.sin(angle) * radius)
    ...
    public static void drawArrow(float x, float y, int color, double width, double length) {
        ...
        try {
            if (Settings.gpuUiRoundedRects != null && Settings.gpuUiRoundedRects.isToggled()) {
                ...
                WorldRenderer worldrenderer = tessellator.getWorldRenderer();
                worldrenderer.begin(GL11.GL_LINES, DefaultVertexFormats.POSITION);
                worldrenderer.pos(x - xOffset, y + yOffset, 0.0D).endVertex();
                worldrenderer.pos(x + length - xOffset, y - length + yOffset, 0.0D).endVertex();
                worldrenderer.pos(x + length - xOffset, y - length + yOffset, 0.0D).endVertex();
                worldrenderer.pos(x + 2 * length - xOffset, y + yOffset, 0.0D).endVertex();
                tessellator.draw();
    ...
    public static void drawRoundedGradientOutlinedRectangle(float n, float n2, float n3, float n4, final float n5, final int n6, final int n7, final int n8) { // credit to the creator of raven b4
        // Optional shader-based path (GPU). Keeps callsites unchanged.
        try {
            if (Settings.gpuUiRoundedRects != null && Settings.gpuUiRoundedRects.isToggled()) {
                final double x = Math.min(n, n3);
                final double y = Math.min(n2, n4);
                final double w = Math.abs(n3 - n);
                final double h = Math.abs(n4 - n2);
                // outline thickness ~1.5px for better visibility (with AA it looks thinner)
                RRectUtils.drawRoundOutlineGradient(x, y, w, h, n5, 1.5, n6, n7, n8);
                return;
            }
        } catch (Throwable ignored) {
        }
    ...
    public static void drawRoundedRectangle(float n, float n2, float n3, float n4, final float n5, final int n6) {
        ...
        try {
            if (Settings.gpuUiRoundedRects != null && Settings.gpuUiRoundedRects.isToggled()) {
                ...
                final double w = Math.abs(n3 - n);
                final double h = Math.abs(n4 - n2);
                RRectUtils.drawRound(x, y, w, h, n5, new Color(n6, true));
                return;
            }
        } catch (Throwable ignored) {
        }
    ...
    public static void drawRoundedGradientRect(float x, float y, float x2, float y2, final float n5, final int n6, final int n7, final int n8, final int n9) {
        ...
        try {
            if (Settings.gpuUiRoundedRects != null && Settings.gpuUiRoundedRects.isToggled()) {
                final double left = Math.min(x, x2);
                final double top = Math.min(y, y2);
                final double w = Math.abs(x2 - x);
                final double h = Math.abs(y2 - y);
                RRectUtils.drawRoundGradient(left, top, w, h, n5, n6, n7, n8, n9);
                return;
            }
        } catch (Throwable ignored) {
        }
    ...
    /**
     * Draws an image with linear texture filtering (smoother scaling).
     * Useful for "photo/high-res" rendering where nearest filtering causes jagged edges.
     *
     * Note: This changes the filter params on the bound texture object.
     */
    public static void drawImageSmooth(ResourceLocation res, float x, float y, float width, float height) {
        RenderUtils.drawImageSmooth(res, x, y, width, height, WHITE);
    }
     ...
     * Note: This changes the filter params on the bound texture object.
     */
    public static void drawImageSmooth(ResourceLocation res, float x, float y, float width, float height, int color) {
        // Alpha test can "clip" soft edges into jaggies; disable it for smooth PNG edges.
        GlStateManager.disableAlpha();
        GL11.glDisable(2929);
        GL11.glEnable(3042);
        GL11.glDepthMask(false);
        ...
        // Force linear sampling to avoid "chainsaw" edges when the HUD is upscaled for photos.
        // If mipmaps exist, this will use trilinear filtering (best-looking for scaled textures).
    ...
    public static void drawBloomShadow(float x, float y, float width, float height, int blurRadius, int roundRadius, int color, boolean scissor, boolean cut_top, boolean cut_bottom, boolean cut_left, boolean cut_right) {
        ...
        int identifier = Objects.hash(Math.round(width), Math.round(height), blurRadius, roundRadius, scissor, cut_top, cut_bottom, cut_left, cut_right);
