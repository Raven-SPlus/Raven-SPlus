...
public class AutoUpdate {
            ...
            switch (result.getType()) {
                ...
                case OLD:
                    Utils.sendMessageAnyWay(ChatFormatting.RED + "You are not at latest version." +
                            ChatFormatting.RESET + " current: " + Version.current() + "  latest: " + result.getLatestVersion());
    ...
    public static void update() {
        ...
        try (CloseableHttpClient httpClient = HttpClientBuilder.create()
            ...
            File file = new File(Raven.mc.mcDataDir + File.separator + "mods", "Raven-S+.jar");
            file.createNewFile();
    ...
    private static @NotNull Result checkVersion() {
        try (CloseableHttpClient httpClient = HttpClientBuilder.create().build()) {
            HttpGet httpGet = new HttpGet("https://api.github.com/repos/Raven-SPlus/Raven-SPlus/releases/latest");
            CloseableHttpResponse response = httpClient.execute(httpGet);
            ...
            // Tag format is b1/b2/...; use GitHub tag_name as-is.
            String ver = jsonObject.get("tag_name").getAsString();
            String url = null;
            try {
                url = jsonObject.getAsJsonArray("assets")
                        .get(0).getAsJsonObject()
                        .get("browser_download_url").getAsString();
            } catch (Exception ignored) {
            }

            // If current >= latest (dev builds, forks, etc.), treat as latest.
            int cmp = Version.compare(Version.current(), ver);
            return new Result(cmp >= 0 ? Result.Type.LATEST : Result.Type.OLD, url, ver);
        } catch (Exception ignored) {
        }
