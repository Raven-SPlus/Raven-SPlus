...
public class RotationUtils {
    ...
    public static float[] fixRotation(float n, float n2, final float n3, final float n4) {
        final float yawDelta = n - n3;
        final float pitchDelta = n2 - n4;
        final float absYawDelta = Math.abs(yawDelta);

        final float sens = mc.gameSettings.mouseSensitivity * 0.6f + 0.2f;
        final double gcd = sens * sens * sens * 1.2;

        // Base quantization to mouse sensitivity GCD
        float qYaw = (float) (Math.round((double) yawDelta / gcd) * gcd);
        float qPitch = (float) (Math.round((double) pitchDelta / gcd) * gcd);

        float outYaw = n3 + qYaw;
        float outPitch = n4 + qPitch;

        // Optional yaw randomization (keep it on the GCD grid to avoid "sensitivity" flags)
        if (absYawDelta >= 1.0f) {
            final int factor = (int) RotationHandler.randomYawFactor.getInput();
            if (factor != 0) {
                final int range = factor * 100 + Utils.randomizeInt(-30, 30);
                outYaw += Utils.randomizeInt(-range, range) / 100.0;
            }
        } else if (absYawDelta > 0.0f && absYawDelta <= 0.04f) {
            // Avoid "always move" drift: only nudge when there is already a non-zero delta.
            // This preserves stable idle rotations while still breaking tiny, repeatable deltas.
            outYaw += Math.copySign(0.01f, yawDelta);
        }

        // Re-quantize after any injected noise so final deltas remain sensitivity-aligned
        outYaw = n3 + (float) (Math.round((double) (outYaw - n3) / gcd) * gcd);
    ...
    public static float normalize(float yaw, float min, float max) {
        ...
        yaw = (float) ((yaw - min) % 360.0F);
        if (yaw < 0) yaw += 360.0F;
        yaw += min;
        while (yaw >= max) yaw -= 360.0F;
        while (yaw < min) yaw += 360.0F;
        return yaw;
    }

    public static float normalize180(float yaw) {
        return normalize(yaw, -180.0F, 180.0F);
    }

    /**
     * Stable yaw comparison with normalization applied to both angles.
     */
    public static boolean isYawClose(float yaw1, float yaw2, float tolerance) {
        return Math.abs(normalize180(yaw1) - normalize180(yaw2)) <= tolerance;
    }
