...
public class Utils {
    ...
    public static final ChatComponentText PREFIX = new ChatComponentText("&6[&6S+&6]&r ");
    public static HashSet<String> friends = new HashSet<>();
    public static HashSet<String> enemies = new HashSet<>();
    public static final Logger log = LogManager.getLogger();
    private static long sidebarCacheTick = -1;
     ...
     * @param stdDev Standard deviation (controls spread)
     * @param min Minimum allowed value
     * @param max Maximum allowed value
     * @return A random value following Gaussian distribution, clamped to [min, max]
     */
    public static double randomizeGaussian(double mean, double stdDev, double min, double max) {
        double value;
        int attempts = 0;
        do {
            value = rand.nextGaussian() * stdDev + mean;
            attempts++;
            // Prevent infinite loop, fallback to uniform if too many attempts
            if (attempts > 100) {
    ...
    /**
     * Generates a random number using Gaussian distribution with automatic bounds
     * Uses 3-sigma rule: 99.7% of values fall within mean ± 3*stdDev
     */
    public static double randomizeGaussian(double mean, double stdDev) {
    ...
    public static void sendMessage(String txt) {
        if (nullCheck()) {
            String m = formatColor("&6[&6S+&6]&r " + replace(txt));
    ...
    public static void sendDebugMessage(String message) {
        if (nullCheck()) {
            mc.thePlayer.addChatMessage(new ChatComponentText("§6[§6S+§6]§r " + message));
        }
    }
    ...
    public static int getBedwarsStatus() {
        ...
        for (String line : getSidebarLines()) {
            ...
            if (sep >= 0) {
                String right = line.substring(sep).trim();
    ...
    public static List<String> getSidebarLines() {
        ...
        if (sidebarCacheTick == currentTick) {
            return cachedSidebarLines;
        }
        ...
        cachedSidebarLines = Collections.unmodifiableList(lines);
        sidebarCacheTick = currentTick;
        return cachedSidebarLines;
    }
    ...
    /**
     * Converts camelCase or PascalCase strings to spaced format.
     * Examples: "KillAura" -> "Kill Aura", "AutoClicker" -> "Auto Clicker", "ClickGUI" -> "Click GUI", "HUD" -> "HUD"
     * If the name already contains spaces, it returns the name as-is.
     */
    public static String formatModuleName(String name) {
        ...
        // If name already contains spaces, return it as-is (assumes it's already formatted)
        if (name.contains(" ")) {
            return name;
        }
        ...
        for (int i = 0; i < name.length(); i++) {
            ...
            // Only add space if previous character is lowercase or digit (to handle consecutive uppercase like "GUI")
