...
public class InvManager extends Module {
    ...
    // Triggered when ChestStealer finishes stealing to run a one-off cleanup
    private int manualTriggerTicks = 0;
    private static final int MANUAL_TRIGGER_DURATION_TICKS = 40;
    ...
    public void onDisable() {
        ...
        state = State.NONE;
        invOpen = false;
        manualTriggerTicks = 0;
    }

    @Override
    public void onUpdate() {
        boolean manualTrigger = manualTriggerTicks > 0;
        if (manualTriggerTicks > 0) {
            manualTriggerTicks--;
        }
        ...
        switch ((int) mode.getInput()) {
            case 0:
                desiredOpen = !(notWhileMoving.isToggled() && MoveUtil.isMoving());
                break;
            case 1:
                desiredOpen = mc.currentScreen instanceof GuiInventory;
                break;
            case 2:
            default:
                desiredOpen = true;
        ...
        // Manual trigger (after chest steal) forces invOpen for a short window regardless of mode constraints
        invOpen = manualTrigger || desiredOpen;
    ...
    /**
     * Request an immediate inventory management pass (used after chest stealing).
     */
    public void triggerAfterSteal() {
        manualTriggerTicks = MANUAL_TRIGGER_DURATION_TICKS;
    }
}