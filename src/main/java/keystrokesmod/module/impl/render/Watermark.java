...
import keystrokesmod.utility.Version;
    ...
    public Watermark() {
        ...
        for (String s : Arrays.asList("default", "enders")) {
            try (InputStream stream = Objects.requireNonNull(Raven.class.getResourceAsStream("/assets/keystrokesmod/textures/watermarks/" + s + ".png"))) {
                ...
                // DynamicTexture only uploads mip level 0. For smooth scaling in high-res photo renders we
                // generate & upload a full mipmap chain ourselves, then use trilinear sampling at draw time.
                final int w = image.getWidth();
                final int h = image.getHeight();
                final int mipLevels = MathHelper.calculateLogBaseTwo(Math.min(w, h));

                DynamicTexture tex = new DynamicTexture(image);
                try {
                    // Allocate storage for all mip levels, then upload generated mip data.
                    TextureUtil.allocateTextureImpl(tex.getGlTextureId(), mipLevels, w, h);
                    int[] level0 = new int[w * h];
                    image.getRGB(0, 0, w, h, level0, 0, w);
                    int[][] mipData = new int[mipLevels + 1][];
                    mipData[0] = level0;
                    mipData = TextureUtil.generateMipmapData(mipLevels, w, mipData);
                    TextureUtil.uploadTextureMipmap(mipData, w, h, 0, 0, true, false);
                } catch (Throwable ignored) {
                    // If mipmap generation isn't supported for some reason, fall back to level-0 texture.
                    tex.setBlurMipmap(true, false);
                }
                WATERMARK.put(s, Minecraft.getMinecraft().renderEngine.getDynamicTextureLocation(s, tex));
        ...
        switch ((int) mode.getInput()) {
            ...
            case 1:
                switch ((int) watermarkPhoto.getInput()) {
                    case 0:
                        RenderUtils.drawImageSmooth(WATERMARK.get("default"), posX, posY, 50, 50);
                        break;
                    case 1:
                        RenderUtils.drawImageSmooth(WATERMARK.get("enders"), posX, posY, 150, 45);
                        break;
                }
                break;
        }
