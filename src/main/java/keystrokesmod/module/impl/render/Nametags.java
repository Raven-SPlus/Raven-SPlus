...
public class Nametags extends Module {
    ...
    private final Map<UUID, CachedName> cachedNames = new HashMap<>();
    public Nametags() {
    ...
    public void onRenderLiving(RenderLivingEvent.Specials.@NotNull Pre e) {
        if (e.entity instanceof EntityPlayer && (e.entity != mc.thePlayer || renderSelf.isToggled()) && e.entity.deathTime == 0) {
            ...
            e.setCanceled(true);
            String name = getCachedName(entityPlayer);
    ...
    private String getCachedName(EntityPlayer player) {
        long tick = mc.theWorld != null ? mc.theWorld.getTotalWorldTime() : 0;
        CachedName cached = cachedNames.get(player.getUniqueID());
        ...
        if (showHealth.isToggled()) {
            name = name + " " + Utils.getHealthStr(player);
        }
        if (showHitsToKill.isToggled()) {
            name = name + " " + Utils.getHitsToKill(player, mc.thePlayer.getCurrentEquippedItem());
        }
        ...
        cachedNames.put(player.getUniqueID(), new CachedName(name, tick));
        return name;
    }
    ...
    private static class CachedName {
        ...
        private CachedName(String text, long tick) {
            this.text = text;
            this.tick = tick;
        }
    }
