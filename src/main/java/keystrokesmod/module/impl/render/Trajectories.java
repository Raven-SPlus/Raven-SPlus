...
public class Trajectories extends Module {
    ...
    public void onRenderWorldLast(RenderWorldLastEvent e) {
        ...
        boolean useTessellator = Settings.gpu3DRendering != null && Settings.gpu3DRendering.isToggled();
        ...
        if (useTessellator) {
            ...
            double currentMotionZ = motionZ;
            
            for (int k = 0; k <= 100 && !ground; ++k) {
                Vec3 start = new Vec3(currentX, currentY, currentZ);
                Vec3 predicted = new Vec3(currentX + currentMotionX, currentY + currentMotionY, currentZ + currentMotionZ);
                MovingObjectPosition rayTraced = mc.theWorld.rayTraceBlocks(start, predicted, false, true, false);
                if (rayTraced != null) {
                    ground = true;
                    target = rayTraced;
                ...
                int vertexColor = (highlight && highlightOnEntity.isToggled()) ? HIGHLIGHT_COLOR : -1;
                float r = ((vertexColor >> 16) & 0xFF) / 255.0f;
                float g = ((vertexColor >> 8) & 0xFF) / 255.0f;
                float b = (vertexColor & 0xFF) / 255.0f;
                
                worldrenderer.pos(currentX - renderPosX, currentY - renderPosY, currentZ - renderPosZ)
                        .color(r, g, b, 1.0f).endVertex();
                
                float f14 = 0.99f;
                currentMotionY *= f14;
                currentX += (currentMotionX *= f14);
                currentY += (currentMotionY -= bow ? 0.05 : 0.03);
                currentZ += (currentMotionZ *= f14);
            }
    ...
    public void drawX() {
        // Optional GPU-friendly path
        if (Settings.gpu3DRendering != null && Settings.gpu3DRendering.isToggled()) {
            Tessellator tessellator = Tessellator.getInstance();
            WorldRenderer worldrenderer = tessellator.getWorldRenderer();
            worldrenderer.begin(GL11.GL_LINES, DefaultVertexFormats.POSITION);
            worldrenderer.pos(-0.25, 0.0, 0.0).endVertex();
            worldrenderer.pos(0.0, 0.0, 0.0).endVertex();
            worldrenderer.pos(0.0, 0.0, -0.25).endVertex();
            worldrenderer.pos(0.0, 0.0, 0.0).endVertex();
            worldrenderer.pos(0.25, 0.0, 0.0).endVertex();
            worldrenderer.pos(0.0, 0.0, 0.0).endVertex();
            worldrenderer.pos(0.0, 0.0, 0.25).endVertex();
