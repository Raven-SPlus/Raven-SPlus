...
public class BedPlates extends Module {
    ...
    private static final Map<Block, ResourceLocation> TEXTURE_CACHE = new HashMap<>();
    ...
    private void drawPlate(BlockPos blockPos, int index) {
        ...
        for (Block block :blocks) {
            mc.getTextureManager().bindTexture(getTextureForBlock(block));
    ...
    private static ResourceLocation getTextureForBlock(Block block) {
        return TEXTURE_CACHE.computeIfAbsent(block, b -> {
            String registry = b.getRegistryName();
            String name;
            if (registry != null) {
                int colon = registry.indexOf(':');
                name = colon >= 0 ? registry.substring(colon + 1) : registry;
            } else {
                name = b.getUnlocalizedName().replace("tile.", "");
            }
            return new ResourceLocation("keystrokesmod:images/" + name + ".png");
        });
    }
