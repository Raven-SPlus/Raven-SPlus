...
public class Notifications extends Module {
    ...
    public static ButtonSetting updateToggleNotif;
    public Notifications() {
        ...
        this.registerSetting(updateToggleNotif = new ButtonSetting("Update toggle notifications", true));
    }
    ...
    public static void sendModuleToggleNotification(NotificationTypes notificationType, String message, String moduleName) {
        if (!ModuleManager.notifications.isEnabled()) return;

        if (!chatNoti.isToggled()) {
            // Check if update feature is enabled and if there's an existing notification for this module
            if (updateToggleNotif.isToggled() && moduleName != null) {
                for (Notification notif : notifs) {
                    if (notif.moduleName != null && notif.moduleName.equals(moduleName)) {
                        // Update existing notification instead of creating a new one
                        notif.updateMessage(message);
                        return;
                    }
                }
            }
            ...
            ScaledResolution sr = new ScaledResolution(mc);
            CoolDown coolDown = new CoolDown(3000);
            coolDown.start();
            AnimationUtils animationX = new AnimationUtils(sr.getScaledWidth());
            animationX.setAnimation(sr.getScaledWidth(), 16);
            notifs.add(new Notification(notificationType,
    ...
    public static class Notification {
        ...
        public String moduleName; // null if not a module toggle notification
        ...
        public Notification(NotificationTypes type, String message, CoolDown duration, AnimationUtils animationX, AnimationUtils animationY) {
            this(type, message, duration, animationX, animationY, null);
        }

        public Notification(NotificationTypes type, String message, CoolDown duration, AnimationUtils animationX, AnimationUtils animationY, String moduleName) {
            this.type = type;
        ...
        public void updateMessage(String newMessage) {
            ...
            // Reset duration to show the updated notification for full duration
            this.duration.setCooldown(3000);
            this.duration.start();
            // Reset animation position to keep notification visible (slide back in if it was sliding out)
            ScaledResolution sr = new ScaledResolution(Minecraft.getMinecraft());
            this.animationX.setAnimation(sr.getScaledWidth() - 125, 16);
        }
    }
}