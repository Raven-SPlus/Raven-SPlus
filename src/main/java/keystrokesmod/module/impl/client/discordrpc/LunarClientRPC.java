...
public class LunarClientRPC extends SubMode<DiscordRpc> {
    ...
    private static final long FETCH_RETRY_MS = TimeUnit.MINUTES.toMillis(1);
    private final Map<String, ServerData> serverDataMap = new ConcurrentHashMap<>();
    private final AtomicBoolean loadingMappings = new AtomicBoolean(false);
    private boolean started;
    private String serverName;
    private String bigImage;
    private String serveraddresses;
    private long lastFetchAttempt;
    ...
    public void onUpdate() {
        ...
        if (!started || (!Utils.nullCheck() && DEFAULT_SERVER_NAME.equals(serverName)) ||
                (currentServerIP != null && serveraddresses != null && !currentServerIP.endsWith(serveraddresses))) {
            if (started) {
                onDisable();
            }
            requestMappingsIfNeeded();
            if (Utils.nullCheck()) {
    ...
    private void requestMappingsIfNeeded() {
        if (!serverDataMap.isEmpty()) {
            return;
        }

        long now = System.currentTimeMillis();
        if (now - lastFetchAttempt < FETCH_RETRY_MS) {
            return;
        }

        if (!loadingMappings.compareAndSet(false, true)) {
            return;
        }
        ...
        Raven.getExecutor().execute(() -> {
            try {
                Map<String, ServerData> loadedMappings = fetchServerMappingsBlocking();
                if (loadedMappings != null && !loadedMappings.isEmpty()) {
                    serverDataMap.clear();
                    serverDataMap.putAll(loadedMappings);
                }
            } finally {
    ...
    private Map<String, ServerData> fetchServerMappingsBlocking() {
        HttpURLConnection connection = null;
        try {
            connection = (HttpURLConnection) new URL(mappingUrl).openConnection();
            ...
            Map<String, ServerData> mappings = new ConcurrentHashMap<>();
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                mappings.put(primaryAddress, serverData);
                ...
                for (int j = 0; j < addressesArray.length(); j++) {
                    ...
                    mappings.put(address, serverData);
                }
            }
