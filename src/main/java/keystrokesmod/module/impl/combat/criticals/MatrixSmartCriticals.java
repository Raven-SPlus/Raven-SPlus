...
public class MatrixSmartCriticals extends SubMode<Criticals> {
    ...
    public MatrixSmartCriticals(String name, @NotNull Criticals parent) {
        super(name, parent);
    ...
    public void onAttack(@NotNull AttackEntityEvent event) {
        if (event.target instanceof EntityLivingBase) {
            ...
            attacks++;
            if (attacks > 3) {
                PacketUtils.sendPacketNoEvent(new C04PacketPlayerPosition(x, y + 0.110314, z, false));
                PacketUtils.sendPacketNoEvent(new C04PacketPlayerPosition(x, y + 0.0200081, z, false));
                PacketUtils.sendPacketNoEvent(new C04PacketPlayerPosition(x, y + 0.00000001300009, z, false));
                PacketUtils.sendPacketNoEvent(new C04PacketPlayerPosition(x, y + 0.000000000022, z, false));
