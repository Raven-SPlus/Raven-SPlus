...
public class BedAura extends Module {
    ...
    private final String[] modes = new String[]{"Legit", "Normal", "Instant", "Swap"};
    ...
    public BedAura() {
        ...
        this.registerSetting(allowAura = new ButtonSetting("Allow aura", true, () -> mode.getInput() != 0));
        ...
        this.registerSetting(rotationSpeed = new SliderSetting("Rotation speed", 20, 2, 20, 0.1));
        this.registerSetting(moveFixMode = new ModeSetting("MoveFix mode", RotationHandler.MoveFix.MODES, 2));
    }
    ...
    public void onDisable() {
        // Release break key if in legit mode
        if (mode.getInput() == 0) {
    ...
    public void onRotation(RotationEvent e) {
        if ((rotate || breakProgress >= 1 || breakProgress == 0) && currentBlock != null) {
            ...
            // Apply rotation speed smoothing (similar to KillAuraV2)
            final double rotationSpeedValue = this.rotationSpeed.getInput();
            if (rotationSpeedValue >= this.rotationSpeed.getMax()) {
                // Instant rotation
                lastYaw = targetYaw;
                ...
                // Smooth rotation using AimSimulator.rotMove (like KillAuraV2)
                lastYaw = AimSimulator.rotMove(targetYaw, lastYaw, rotationSpeedValue);
                lastPitch = AimSimulator.rotMove(targetPitch, lastPitch, rotationSpeedValue);
                e.setYaw(lastYaw);
                e.setPitch(lastPitch);
            }
            
            // Apply MoveFix (like KillAuraV2)
            e.setMoveFix(RotationHandler.MoveFix.values()[(int) moveFixMode.getInput()]);
            
            rotate = false;
        } else {
            // Reset last rotations when not rotating
            lastYaw = e.getYaw();
            lastPitch = e.getPitch();
        }
    }
    ...
    private void resetSlot() {
        if (currentSlot != -1 && currentSlot != mc.thePlayer.inventory.currentItem && mode.getInput() == 3) {
            stopAutoblock = true;
            delayStart = true;
    ...
    private double getEfficiency(BlockPos pos) {
        ...
        ItemStack tool = (mode.getInput() == 3 && Utils.getTool(block) != -1) ? mc.thePlayer.inventory.getStackInSlot(Utils.getTool(block)) : SlotHandler.getHeldItem();
    ...
    private void breakBlock(BlockPos blockPos) {
        ...
        if (modeValue == 0) {
            // Legit mode: Keep holding break key until bed breaks (using auto clicker method)
            if (breakProgress == 0) {
                resetSlot();
                stopAutoblock = true;
                rotate = true;
                setSlot(Utils.getTool(block));
                // Hold the break key using auto clicker method
                Utils.sendClick(0, true);
            }
            // Continue holding break - don't stop until it breaks
            if (breakProgress >= 1) {
                // Release the break key
                Utils.sendClick(0, false);
                reset(false);
                stopAutoblock = true;
                delayStart = true;
            ...
            // Keep holding the break key and rotating
            Utils.sendClick(0, true);
            stopAutoblock = true;
            rotate = true;
            double progress = vanillaProgress = (float) (BlockUtils.getBlockHardness(block, SlotHandler.getHeldItem(), false, ignoreSlow.isToggled() || groundSpoof.isToggled()) * breakSpeed.getInput());
            ...
            // Normal mode: Ported from old legit mode logic
            if (breakProgress == 0) {
                resetSlot();
                stopAutoblock = true;
                rotate = true;
            ...
            double progress = vanillaProgress = (float) (BlockUtils.getBlockHardness(block, (Utils.getTool(block) != -1) ? mc.thePlayer.inventory.getStackInSlot(Utils.getTool(block)) : SlotHandler.getHeldItem(), false, ignoreSlow.isToggled() || groundSpoof.isToggled()) * breakSpeed.getInput());
            if (lastProgress != 0 && breakProgress >= lastProgress) {
                ModuleManager.killAura.resetBlinkState(false);
                stopAutoblock = true;
            }
